...
**Version:** 2.0 | **Date:** September 2026 | **Author:** Enterprise Architecture Team
...
This document describes the enterprise architecture for the simplified **Freddie Mac-style Home Loan Application and Customer Management Platform**. The system supports end-to-end mortgage lifecycle operations including:

- **Auth & Identity** — User authentication, JWT token issuance
- **Customer Management** — Borrower profile management, KYC identity verification
- **Loan Origination** — Customer application intake and status workflow management
- **Underwriting** — Automated risk scoring, DTI/LTV calculation, and loan decisioning
- **Document Management** — Loan document upload and verification

### Key Architectural Principles
| Principle | Implementation |
|---|---|
| Domain-Driven Design | Bounded contexts across 7 core microservices |
| API-First | OpenAPI/Swagger contracts define all interfaces |
| Spring Boot 3.3 | Modern Java 17/21 runtime with Spring Cloud Gateway & Eureka |
| Polyglot Persistence | PostgreSQL 16 (`freddie_customer` & `freddie_loans`) |
| Fail-Safe | Circuit breakers & fallback endpoints |
| Zero-Trust Security | OAuth2 + JWT authentication via `auth-service` |

---

## 2. Architecture Overview

### Architecture Style: Microservices Architecture (7 Core Modules)

```
┌─────────────────────────────────────────────────────────────────────┐
│                        PRESENTATION LAYER                           │
│  Angular Portal Client / Postman / Mobile Client                    │
└────────────────────────────┬────────────────────────────────────────┘
...
└────────┬──────────┬──────────┬──────────┬──────────┬───────────────┘
    ...
    REST/HTTP  REST/HTTP  REST/HTTP  REST/HTTP  REST/HTTP
         │          │          │          │          │
┌────────▼──┐ ┌─────▼───┐ ┌───▼────┐ ┌──▼─────┐ ┌──▼──────────┐
│   Auth    │ │Customer │ │ Loan   │ │Under-  │ │ Document   │
│ Service   │ │ Service │ │Origina-│ │writing │ │ Service    │
│  (:8085)  │ │ (Post-  │ │tion Svc│ │Service │ │ (Post-     │
│           │ │ gresql) │ │ (Post- │ │ (Post- │ │ gresql)    │
│           │ │  :8081  │ │ gresql)│ │ gresql)│ │   :8084    │
│           │ │         │ │  :8082 │ │  :8083 │ │            │
└───────────┘ └─────────┘ └────────┘ └────────┘ └────────────┘
```
...
graph TB
    ...
    subgraph "Databases"
        PG1["🐘 PostgreSQL (freddie_customer)\nCustomer Profiles & Docs"]
        PG2["🐘 PostgreSQL (freddie_loans)\nLoan Apps & Underwriting"]
    end
    ...
    UI -->|HTTPS / JWT| GW
    GW --> AUTH & CS & LOS & UWS & DS
    GW -.->|Service Discovery| SD
    AUTH & CS & LOS & UWS & DS -.->|Register| SD
